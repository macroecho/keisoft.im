...
namespace Keisoft.IM.Http.Controllers
{
    ...
    public class IdentityRpcController
    {
        private readonly IIdentityService _identityService;

        public IdentityRpcController(IIdentityService identityService)
        {
            _identityService = identityService;
        }

        [HttpGet, Route("Token/{uid}/{platform}/{deviceNo}/{timeStamp}")]
        public ActionResult Token([FromRoute] int uid, [FromRoute] byte platform, [FromRoute] string deviceNo, [FromRoute] long timeStamp)
        {
            if (uid <= 0)
            {
                throw new ArgIllegaException(nameof(uid));
            }

            if (string.IsNullOrEmpty(deviceNo))
            {
                throw new ArgIllegaException(nameof(deviceNo));
            }

            return _identityService.Create(uid, (PlatformTypeEnum)platform, deviceNo, timeStamp).ToJson();
        }
    }
}
