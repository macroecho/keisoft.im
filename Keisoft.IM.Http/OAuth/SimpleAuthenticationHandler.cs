...
{
    internal class SimpleAuthenticationHandler : AuthenticationHandler<SimpleOptions>
    {
        ...
        protected override Task<AuthenticateResult> HandleAuthenticateAsync()
        {
            try
            {
                ...
                var schemeAndParameter = authorization.Split(' ');
                var parameter = schemeAndParameter.Length == 1 ? schemeAndParameter[0] : schemeAndParameter[1];

                var result = _identityService.ToIdentityOutput(parameter);

                var claims = new List<Claim>
                {
                    new Claim(ClaimTypes.Sid, result.UId.ToString())
                };

                var identity = new ClaimsIdentity(claims, SimpleOptions.Scheme);
                var identities = new List<ClaimsIdentity> { identity };
                var principal = new ClaimsPrincipal(identities);
                var ticket = new AuthenticationTicket(principal, SimpleOptions.Scheme);
