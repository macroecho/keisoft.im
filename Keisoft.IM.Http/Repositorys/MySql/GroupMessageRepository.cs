...
namespace Keisoft.IM.Http.Repositorys.MySql
{
    internal class GroupMessageRepository : IGroupMessageRepository
    {
        ...
        async Task<int> IGroupMessageRepository.CountAsync(CancellationToken cancellationToken)
        {
            ...
            var dr = await db.ExecuteScalarAsync("SELECT COUNT(*) FROM group_message", cancellationToken);
        ...
        async Task<int> IGroupMessageRepository.CountAsync(long startTimeStamp, long endTimeStamp, CancellationToken cancellationToken)
        {
            ...
            (
                $"SELECT COUNT(*) FROM group_message WHERE `time_stamp` >= {startTimeStamp} AND `time_stamp` < {endTimeStamp}",
                cancellationToken
            );
        ...
        async Task<MessageCountAndMaxId> IGroupMessageRepository.CountAndMaxIdAsync(int ownerUId, long startId, CancellationToken cancellationToken)
        {
            ...
            (
                $@"
                    SELECT 
                            COUNT(a.id) AS A, 
                            IFNULL(MAX(a.id),0) AS B 
                    FROM 
                            group_message AS a 
                                LEFT JOIN 
                            group_member AS b ON(b.member_id = { ownerUId } AND a.group_id = b.group_id AND a.time_stamp >= b.time_stamp)
                    WHERE 
                            a.id > { startId } AND((a.`to` = 0 AND b.member_id = { ownerUId }) OR a.`to` = { ownerUId });",
                cancellationToken
            );
        ...
        async Task<List<GroupMessage>> IGroupMessageRepository.GetListAsync(int ownerUId, long startId, int limit, CancellationToken cancellationToken)
        {
            ...
            // ((a.`to` = 0 AND b.member_id = { ownerUId }) OR a.`to` = { ownerUId }) 

            using var db = new DbAccess(_connectionString);
            using var dr = await db.ExecuteReaderAsync
            (
                $@"SELECT 
                        a.`id`, a.`cid`, a.`from`, a.`to`, a.`group_id`, a.`type`, a.`etype`, a.`content`, a.`time_stamp`, a.`encrypt` 
                   FROM 
                        group_message AS a 
                           LEFT JOIN 
                        group_member AS b ON(b.member_id = { ownerUId } AND a.group_id = b.group_id AND a.time_stamp >= b.time_stamp)
