...
using Keisoft.IM.Server.Sdk.Options;

using Keisoft.IM.Http.Utilities;

namespace Keisoft.IM.Http
{
    public static class IMSdkExtensions
    {
        public static IServiceCollection AddIMSdk(this IServiceCollection services, Action<SdkOptions> configure)
        {
            var sdkOptions = new SdkOptions();

            services.AddSingleton(sdkOptions);
            services.AddSingleton<IMSdkLogger>();

            configure?.Invoke(sdkOptions);

            return services;
        }

        public static IApplicationBuilder UseIMSdk(this IApplicationBuilder builder)
        {
            var sdkOptions = builder.ApplicationServices.GetService<SdkOptions>();
            var logger = builder.ApplicationServices.GetService<IMSdkLogger>();

            sdkOptions.Logger = logger;

            // 设置通讯公钥证书。
            

            // 初始化 IM 组件服务。
            IMSdk.Init(sdkOptions);
            IMSdk.ConnectAsync(sdkOptions.Token);
